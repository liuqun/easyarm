...
//!
//! This function is contained in <tt>utils/flash_pb.c</tt>, with
//! <tt>utils/flash_pb.h</tt> containing the API definition for use by
//! applications.
//!
...
//! in \ref FlashPBSave()) and the second byte is a checksum used to validate
//! the correctness of the data (the checksum byte is the byte such that the
//! sum of all bytes in the parameter block is zero).
//!
